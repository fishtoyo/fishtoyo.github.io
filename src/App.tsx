...
function ScrollMouse({ isLast = false }: { isLast?: boolean }) {
  ...
  return (
    <motion.div 
      ...
      className={`fixed z-50 right-7.5 ${isLast ? 'bottom-7.5' : 'bottom-12.5'} min-[744px]:right-16.25 ${isLast ? 'min-[744px]:bottom-17.5' : 'min-[744px]:bottom-20'} w-7 h-9.5 border-2 border-white rounded-[100px] flex justify-center pt-2.5 cursor-pointer transition-all duration-300 before:content-[''] before:absolute before:-top-4 before:left-[calc(50%-6px)] before:w-3 before:h-3 before:border-t-2 before:border-l-2 before:border-white before:rotate-45 ${!isLast ? "after:content-[''] after:absolute after:-bottom-4 after:left-[calc(50%-6px)] after:w-3 after:h-3 after:border-b-2 after:border-r-2 after:border-white after:rotate-45" : ""}`}
    >
      {/* 滾輪 */}
...
function App() {
  ...
  useEffect(() => {
    const handleGlobalScroll = () => {
      ...
      // 當往下滾動超過畫面高度 30% 時顯示，避免跟 Welcome 區塊本身的圈圈文字圖示重疊
      setShowScrollMouse(scrollY > innerHeight * 0.3);
      // 判斷是否抵達最底部 (容許 50px 小誤差)
      setIsAtBottom(Math.ceil(scrollY + innerHeight) >= scrollHeight - 50);
    };
  ...
  return (
    ...
    >
      ...
      <header className="fixed top-0 left-0 w-full z-100 flex justify-between items-center pt-5 px-7.5 min-[744px]:pt-17.5 min-[744px]:px-16.25 bg-transparent transform-gpu will-change-transform">
        ...
        <div className="flex gap-2 md:gap-6 text-sm font-bold text-white/90 drop-shadow-md">
      ...
      <section className="relative z-1 min-h-screen lg:min-h-0 lg:h-screen w-full flex flex-col justify-center p-6 pt-20 pb-45 md:p-12 md:pt-12 lg:p-[clamp(2rem,5vh,6rem)] lg:pt-[clamp(2rem,5vh,6rem)] overflow-hidden snap-start">
      ...
      >
        <div className="content-container">
          ...
          >
            {worksList.map((work) => (
              <div key={work.id} className="shrink-0 snap-start w-full md:w-[calc(50%_-_20px)] min-[1481px]:w-[calc(100%/3_-_80px/3)]">
                <div 
                  className="group aspect-640/480 bg-white rounded-[20px] mb-3 lg:mb-4 shadow-[2px_2px_5px_rgba(0,0,0,0.1)] overflow-hidden transition-colors relative cursor-pointer"
                ...
                <div className="px-2.5 flex justify-between items-center gap-3">
                  <div className="flex-1 flex items-baseline gap-2.5 overflow-hidden">
                    <h3 className="text-[20px] lg:text-[22px] font-bold text-white whitespace-nowrap shrink-0">{work.translations[lang].title}</h3>
                    <span className="text-white/40 text-[14px] lg:text-[16px] shrink-0 font-medium">/</span>
                    <p className="text-[14px] lg:text-[15px] font-medium text-white/70 truncate">
                      {work.translations[lang].desc}
                    </p>
                  </div>
                  {work.link && (
                    <a 
                      ...
                      className="shrink-0 flex items-center justify-center gap-1.5 px-3 py-1.5 bg-white/10 hover:bg-white/20 border border-white/20 rounded-md transition-colors text-white text-[13px] font-medium cursor-pointer"
                      title={lang === 'en' ? 'Visit Website' : '前往網站'}
                    >
                      <span className="material-symbols-outlined text-[16px]">open_in_new</span>
                      <span className="hidden md:inline">{lang === 'en' ? 'Link' : '前往'}</span>
                    </a>
                  )}
                </div>
              </div>
      ...
      <AnimatePresence>
        {showScrollMouse && <ScrollMouse isLast={isAtBottom} />}
      </AnimatePresence>
